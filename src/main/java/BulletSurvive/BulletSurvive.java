...
public class BulletSurvive {
	...
	public void signalLevel(LEVEL level, int score) {
		this.level_no = level;
		this.level_signal = true;
		this.score = score;
	}
	...
	private void init() {
		...
		base_shader =
				new Shader("assets/shaders/base.vert", "assets/shaders/base.frag");

		// Set pixel matrix
		pixelMatrix =
				new Matrix4f()
						.scale(1 / (getDimensions().x / 2.f), 1 / (getDimensions().y / 2.f), 1);
	...
	private void initWindowIcon() {
		try (MemoryStack stack = stackPush()) {
			...
			if (image == null) throw new RuntimeException(
					String.format("Image not found: %s", icon)
			);
		...
		while (!glfwWindowShouldClose(window)) {
			...
			if (this.level_signal) {
				...
				switch (this.level_no) {
					...
					// Game over
					case GAME_OVER:
						this.level = new GameOver(this.score);
						break;
				}
			}
		}
